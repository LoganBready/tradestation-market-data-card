...
- Replaced all inline styles on the demo page with a dedicated CSS Module (`styles/demo.module.css`) — the generated code used inline styles on wrapper elements like `<main>`, headings, and section labels. Inline styles bypass the design token system and are inconsistent with how the rest of the project is styled. Moving to a CSS Module means all values flow through `var(--token-name)` and the demo page follows the same conventions as the component.
- Replaced `.then()` chaining in `useEffect` with `async/await` and `try/catch` — more readable and idiomatic modern JavaScript. Used a `finally` block to call `setIsLoading(false)` in one place rather than duplicating it in both the success and error branches.
